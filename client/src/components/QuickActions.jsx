...
const QuickActions = ({ onCreateOrder, onAddDriver }) => {
  ...
  const handleAction = (action) => {
    if (action.label === 'Create Order' && onCreateOrder) onCreateOrder();
    if (action.label === 'Add Driver' && onAddDriver) onAddDriver();
    setIsOpen(false);
  };
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute top-14 right-0 w-[200px] bg-card-bg rounded-2xl shadow-2xl border border-border-main p-2 z-50 origin-top-right flex flex-col gap-1"
            >
              ...
              {actions.map((action, i) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-3 w-full text-left px-3 py-2.5 rounded-xl hover:bg-surface-bg transition-colors group"
                  >
